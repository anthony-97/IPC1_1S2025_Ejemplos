...
public class Controlador {
    ...
    public void crearVentanas() { //Metodo que crea las ventanas y les setea el controlador
        ...
        this.vista.setVentanaReportes(new VentanaReportes());
        this.vista.getVentanaReportes().setControlador(this);
    }
    ...
    public void mostrarVentana(TipoVentana.Ventana tipoVentana) {
        ...
        switch (tipoVentana.getIndex()) {
            ...
            case 6 -> this.vista.getVentanaReportes().setVisible(true);
        }
    }
    ...
    /*---------------------Control de acciones iniciales----------------------*/
    
    public void crearAdminYMecanicos() {
        Administrador admin = new Administrador("3056670410301", "Admin", "admin", "1234");
        //Como no se deserializa, se debe crear la lista de mecanicos cada vez
        this.modelo.setListaMecanicos(new Vector<Mecanico>());
        ...
        //Se crea al admin si es la primera ejecucion y no se ha deserializado la lista de usuarios
        boolean adminCreado = false;
        for(Usuario u: this.modelo.getListaUsuarios()) {
            if(u.getDpi().equals(admin.getDpi())) {
                adminCreado = true;
            }
        }
        if(!adminCreado) this.modelo.getListaUsuarios().add(admin);
    }
    
    public void crearVentanaProgresoClientes() {
    ...
    /*---------------------Registro manual y cargas masivas----------------------*/
    ...
    public void imprimirSerializados() {
        ...
        this.modelo.imprimirRepuestos();
        this.modelo.imprimirServicios();
    }

    /*---------------------Control de los reportes----------------------*/
    
    public void generarReporteClientes() {
        ReportadorControlador rep = new ReportadorControlador();
        rep.setModelo(this.modelo);
        rep.generarClientes();
    }
